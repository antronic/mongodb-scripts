...
let summaryInformation = { databases: {}, time: 0 };
let _system_confing = {
    startDateTime: dayjs(),
    endDateTime: dayjs(),
};
...
function generateArchiveQueryToS3(database, _excludeCollections = [], customFolderStructure) {
    ...
    function setupMonthRange() {
        ...
        const monthRange = dayjs(_endDate).diff(_startDate, 'month');
    ...
    for (const collection of collections) {
        ...
        databasesResult = Object.assign(Object.assign({}, databasesResult), { [`${database}.${collection}`]: monthBatches.map((batch, index) => {
                console.log(`Running batch ${database}.${collection}... [${index + 1}/${monthBatches.length}]`);
                return batch();
            }) });
        summaryInformation.databases = Object.assign(Object.assign({}, summaryInformation.databases), { [database]: [...summaryInformation.databases[database], collection] });
        console.log('==========================');
    }
    // return results
...
function startArchive() {
    ...
    for (const [db, colls, folder] of archiveDatabases) {
        summaryInformation.databases = Object.assign(Object.assign({}, summaryInformation.databases), { [db]: [] });
